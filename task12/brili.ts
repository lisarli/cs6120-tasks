...
function evalInstr(instr: bril.Instruction, state: State): Action {
  ...
  if(traceLen < maxTraceLen || (traceLen == maxTraceLen && numCall > 0)){
    if(numCall == 0){
      traceLen++;
      lastOffset++;
  ...
  switch (instr.op) {
    ...
    case "jmp": {
      let label = getLabel(instr, 0);
      if(updateLabel){
        lastLabel = label;
      }
      return { "action": "jump", "label": label };
    }
...
async function main() {
  try {
    ...
    evalProg(prog);
    console.error(lastLabel);
    console.error(lastOffset);
  } catch (e) {
